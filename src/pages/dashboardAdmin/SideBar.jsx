...
const SideBar = ({setActivePage, isSidebarExpanded}) => {
  return(
    <>
      <div
      id="sidebar"
      className={`bg-white h-screen shadow-xl px-3 w-30 overflow-x-hidden transition-transform duration-300 ease-in-out ${isSidebarExpanded ? 'w-60' : 'w-16'}`}
    >
        ...
        <div id="menu" className="flex flex-col space-y-2">
          {menuItems.map((item, index) => (
            <div
              ...
              className="text-sm font-medium text-gray-700 py-2 px-2 hover:bg-sky-500 hover:text-white hover:text-base rounded-md transition duration-150 ease-in-out"
            >
              <svg
              ...
              <span className={`ml-2 ${!isSidebarExpanded ? 'hidden' : ''}`} >{item.title}</span>
            </div>
          ))}
        </div>
      </div>
